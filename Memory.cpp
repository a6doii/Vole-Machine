...
void Memory :: set_memory (string start ,  vector<string> lines , map<string , string > &lines_step , bool step  ){ // ( A0 ) // 0F ==>  15 ==> F ==> 0F
    ...
    for (int i =0 ;i < lines.size( ) ; i++ ){
        ...
        lines_step [start] = first_half; // put the first one inside the address start 
        int integer_address = Hexa_to_decimal ( start) + 1 ;
        start = decimal_to_hexa( integer_address); // A1 
        if ( start.size() == 1 )
        {
